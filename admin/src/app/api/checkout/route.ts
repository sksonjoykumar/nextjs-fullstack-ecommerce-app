...
export const POST = async (req: NextRequest) => {
  try {
    ...
    // check if cartItems and customer
    if (!cartItems || !customer) {
      return NextResponse.json(
        { message: "Not enough data to checkout" },
        { status: 400 },
      );
    }
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      shipping_options: [
        { shipping_rate: "shr_1RvUXU2fCYivmC0Y6jepD4ph" },
        { shipping_rate: "shr_1RvUVd2fCYivmC0YOVcRIeaq" },
      ],
